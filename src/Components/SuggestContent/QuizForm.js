...
export default function QuizForm() {
    ...
    return (
        <div className='quiz-form-container'>
            <p className='quiz-form-p'>Do you have a quiz question that you'd like to have featured on the site? If so, send in a suggestion. </p>
            <form className='quiz-form' onSubmit={handleSubmit(onSubmit)}>
                ...
                ></input>
                {errors.quiz_topic &&
                    errors.quiz_topic.type === "required" && (
                        <p className="error">Quiz topic must not be blank</p>
                    )}
                {errors.quiz_topic &&
                    errors.quiz_topic.type === "minLength" && (
                        <p className="error">
                            Quiz topic must have at least 3 characters
                        </p>
                    )}
                ...
                {errors.quiz_question &&
                    errors.quiz_question.type === "required" && (
                        <p className="error">Quiz question must not be blank</p>
                    )}
                {errors.quiz_question &&
                    errors.quiz_question.type === "minLength" && (
                        <p className="error">
                            Quiz question must have at least 20 characters
                        </p>
                    )}
                <button id='quiz-button' type="submit"> Submit </button>
            </form>
        </div>
    )
}
